...
deduplicate, and attaches results to the shared context.
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# Prompts for LLM sub-agents (Groq Llama 3.3 / Gemini Flash)
# ═══════════════════════════════════════════════════════════════════════════
...
async def _call_llm(prompt: str, model: str = "gemini-flash") -> str:
    ...
    * ``groq-llama``       → Groq Llama 3.3 (deep analysis)
    """
    if model == "gemini-flash":
        return await _call_gemini_flash(prompt)
    return await _call_groq_llama(prompt)
...
async def _call_groq_llama(prompt: str) -> str:
    import os
    import httpx
...
class CodeSmellDetector:
    ...
    async def run(self, snapshot: RepoSnapshot) -> List[AuditFinding]:
        ...
        if not top_files:
            ...
            exts = {".py", ".js", ".ts", ".jsx", ".tsx", ".go", ".rs", ".java", ".rb", ".ipynb"}
            top_files = [
        ...
        files_text_list = []
        for path in top_files[:10]:
            file_obj = snapshot.files.get(path)
            if file_obj and file_obj.content:
                snippet = file_obj.content[:3000]
                files_text_list.append(f"### File: {path}\n(changed {file_counts.get(path, 0)} times)\n```\n{snippet}\n```")
            else:
                files_text_list.append(f"### File: {path}\n(changed {file_counts.get(path, 0)} times)\n// Content not available in snapshot")

        files_text = "\n\n---\n\n".join(files_text_list)
        prompt = _CODE_SMELL_PROMPT.format(files=files_text)
        raw = await _call_llm(prompt, model="groq-llama")
        try:
            arr = json.loads(_extract_json(raw))
        except json.JSONDecodeError:
            arr = []
...
class TestCoverageAnalyzer:
    ...
    async def run(self, snapshot: RepoSnapshot) -> List[AuditFinding]:
        ...
        source_exts = {".py", ".js", ".ts", ".jsx", ".tsx", ".go", ".rs", ".java", ".rb", ".php", ".ipynb"}
...
class SecurityScanner:
    ...
    async def _llm_scan(self, snapshot: RepoSnapshot) -> List[AuditFinding]:
        snippets = []
        for path, file_obj in snapshot.files.items():
            if file_obj.content:
                snippets.append(f"### File: {path}\n```\n{file_obj.content[:3000]}\n```")
        
        if not snippets:
            for path in snapshot.file_tree[:15]:
                if any(path.endswith(e) for e in [".py", ".js", ".ts", ".go", ".rs", ".java", ".ipynb"]):
                    snippets.append(f"### File: {path}\n// Review manually")
